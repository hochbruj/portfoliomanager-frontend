...
class CategoryChart extends PureComponent {
  ...
  getData(category) {
    ...
    if (positions && values && dates) {
      let positionsFil = positions.filter(p => p.category === category)
      let date = dates[dates.length - 1]
      // get all items
      Array.from(new Set(positionsFil.map(s => s.item))).forEach((item) => {
        let positionIds = positionsFil.reduce((ids, position) => {
        ...
        if (category === 'Stock Short') {
          valuesFilAmount = valuesFilAmount * -1
        }
        let dataElement = {
          name: item,
    ...
    return (
      <Fragment>
        <Typography component="h2" variant="h6" color="primary">
          {this.props.category} asset allocation
                      </Typography>
        <PieChart width={450} height={400} >
          <Pie
            activeIndex={this.state.activeIndex}
            activeShape={renderActiveShape}
            data={this.getData(this.props.category)}
            cx={200}
            cy={200}
            innerRadius={60}
            outerRadius={80}
...
export default
  connect(mapStateToProps)(CategoryChart);