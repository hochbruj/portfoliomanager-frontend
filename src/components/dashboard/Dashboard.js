...
import { getPositions, getValues, getChart } from '../../store/actions/positionActions'
...
class Positions extends Component {
  constructor(props) {
    super(props);
    this.state = {totalAmount: 0, date: '', weekDiff: 0,
       weekDiffPerc: 0, yearDiff: 0, yearDiffPerc: 0};
  }
  ...
  componentDidUpdate() {
    if (this.props.positions.length > 0 && this.props.values.length > 0) {
      const { data, totalAmount, date, weekDiff, weekDiffPerc, yearDiff, yearDiffPerc } = this.getSummary()
      if (this.state.totalAmount === 0) {
      this.setState({ totalAmount, date, weekDiff, weekDiffPerc, yearDiff, yearDiffPerc })
  ...
  getSummary() {
    let data = []
    let totalAmount = 0, weekDiff = 0, weekDiffPerc = 0, yearDiff= 0, yearDiffPerc = 0
    let lastDate = ''
  ...
  getText(diff, diffPerc, classes) {
    if (diff >= 0) {
      return <Typography  style={{ color: '#4caf50' }}>
        <ArrowUpward className={classes.iconUp} />
        ${diff.toLocaleString()} ({diffPerc}%)</Typography>
    } else {
      return <Typography style={{ color: '#f44336' }}>
  ...
  render() {
    ...
    const { totalAmount, date, weekDiff, weekDiffPerc, yearDiff, yearDiffPerc } = this.state
            ...
            <Grid item xs={12}>
              <Paper className={classes.paper}>
                ...
                {positions && <Table size="small">
                  ...
                  <TableBody>
                    {Array.from(new Set(positions.map(s => s.category))).map(category => (
                      <CategoriesRow key={category} category={category} totalAmount={totalAmount} />))}
                  </TableBody>
                </Table>}
              </Paper>
            </Grid>
...
export default
  connect(mapStateToProps, mapDispatchToProps)(withStyles(styles)(Positions));