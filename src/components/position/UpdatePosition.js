...
const initialState = {
  category: '', item: '', amount: '', location: '', positionId: '',
  errors: null
};
...
class UpdatePosition extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    if (nextProps.position && nextProps.position !== this.props.position) {
      this.setState({ category: nextProps.position.category,
    ...
    return (
      <Dialog  open={open} onClose={onClose} onExited={this.handleExited} onKeyPress={this.handleKeyPress}>
        ...
        <DialogActions>
          ...
          <Button color="primary" disabled={( !category || !item || !amount || !location)} variant="contained" onClick={this.handleUpdateClick}>Update amount</Button>
        </DialogActions>
      </Dialog>
    );
  }
...
export default connect(null, mapDispatchToProps)(withStyles(styles)(UpdatePosition))